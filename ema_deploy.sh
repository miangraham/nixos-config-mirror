...
mkdir -p ./builds
nix build .#nixosConfigurations.ema.config.system.build.toplevel --out-link ./builds/ema
OUTPUT=$(readlink ./builds/ema)
echo "Built: ${OUTPUT}"

# Copy

nix copy "${OUTPUT}" --to ssh://ian@ema
...
ssh -t ema "sudo sh -c \"${SWITCH_CMD}\""

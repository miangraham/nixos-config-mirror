...
mkdir -p ./builds
nix build .#nixosConfigurations.anzu.config.system.build.toplevel --out-link ./builds/anzu
OUTPUT=$(readlink ./builds/anzu)
echo "Built: ${OUTPUT}"

# Copy

nix copy "${OUTPUT}" --to ssh://ian@anzu
...
ssh -t anzu "sudo sh -c \"${SWITCH_CMD}\""
